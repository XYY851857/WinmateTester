...
def set_static_ip() -> None:
    ...
    try:
        ...
        print("「乙太網路」介面設置為靜態 IP：192.168.255.100/255.255.0.0，閘道：192.168.0.1")
    except subprocess.CalledProcessError as e:
        print("設定靜態 IP 失敗")
        print(e.stderr)
...
if __name__ == "__main__":
    # Wi-Fi 名稱與 XML 設定檔路徑
    wifi_name = "WM_Tester"
    path = f".\\0\\Winmate_Test_GUI\\exes_simu\\Wi-Fi-{wifi_name}.xml"

    # 組合要執行的 PowerShell 指令
    ps_commands = f"""
        netsh interface ipv4 set address name="乙太網路" source=static 192.168.3.101 255.255.252.0 192.168.0.1
        netsh interface ipv4 set address name="乙太網路 2" source=static 192.168.3.102 255.255.252.0 192.168.0.1
        netsh interface ipv4 set address name="Wi-Fi 2" source=static 192.168.3.103 255.255.252.0 192.168.0.1
    ...
    threads.append(
        threading.Thread(
            ...
            args=("192.168.3.101", "192.168.3.103", info_data["192.168.3.101"])
        )
    )
    threads.append(
        threading.Thread(
            target=ping_with_source,
            args=("192.168.3.102", "192.168.3.103", info_data["192.168.3.102"])
        )
    )
    threads.append(
        threading.Thread(
            target=ping_with_source,
            args=("192.168.3.103", "192.168.3.101", info_data["192.168.3.103"])
        )
    )
